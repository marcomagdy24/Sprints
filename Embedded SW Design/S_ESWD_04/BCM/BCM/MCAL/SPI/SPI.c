...
uint8_t SPI_Init(uint8_t SpiNumber) 
{
	...
	if (SpiNumber <= NumOfSPIChannels) 
	{
		...
		switch (SPI_Mode) 
		{
			...
			DIO_SetPinDirection(SPI_PORT, SPI_SS_PIN, OUTPUT);
			DIO_WritePin(SPI_PORT, SPI_SS_PIN,HIGH);
			DIO_SetPinDirection(SPI_PORT, SPI_SCK_PIN, OUTPUT);
			DIO_SetPinDirection(SPI_PORT, SPI_MOSI_PIN, OUTPUT);
			/*selecting the clock mode*/
			MODIFY_REG(SPI_ControlReg, ClockPhase_MSK,
			Get_Bit(SPI_ClockMode,0)<<ClockPhase_BIT);
			MODIFY_REG(SPI_ControlReg, ClockPolarity_MSK,
			Get_Bit(SPI_ClockMode,1)<<ClockPolarity_BIT);
			...
			Get_Bit(SPI_Freq,2)<<ClockSelectBit_2_BIT);
			MODIFY_REG(SPI_ControlReg, ClockSelectBit_1_MSK,
			Get_Bit(SPI_Freq,1)<<ClockSelectBit_1_BIT);
			MODIFY_REG(SPI_ControlReg, ClockSelectBit_0_MSK,
			Get_Bit(SPI_Freq,0)<<ClockSelectBit_0_BIT);
			/*clearing flags*/
			Dummy = Get_Bit(SPI_StatusReg,WriteCollisionFlag_BIT);
			Dummy = Get_Bit(SPI_StatusReg,InterruptFlag_BIT);
			...
			/*SPI PIN Initialization*/
			DIO_SetPinDirection(SPI_PORT, SPI_MISO_PIN, OUTPUT);
			/*selecting the clock mode*/
			MODIFY_REG(SPI_ControlReg, ClockPhase_MSK,
			Get_Bit(SPI_ClockMode,0)<<ClockPhase_BIT);
			MODIFY_REG(SPI_ControlReg, ClockPolarity_MSK,
...
uint8_t SPI_ReceiveChar(uint8_t SpiNumber, ptr_uint8_t RxData, uint8_t slave_CH) {
	...
	switch (SPI_Mode) {
		...
		if (slave_CH <= NumOfSPISlaves) {
			...
			if ((Get_Bit(SPI_StatusReg, InterruptFlag_BIT))) {
				/*Read Data from data_register*/
				*RxData = SPI_DataReg;
				DIO_WritePin(SlaveID_1_Port, SlaveID_1_Pin, HIGH);
				ErrRetVal = OperationSuccess;
				} else {
				ErrRetVal = OperationFail;
			}
...
uint8_t SPI_TransmitString(uint8_t SpiNumber, ptr_uint8_t TxString, uint8_t slave_CH) 
{
	...
	while(iterator <= 255)
	{
		if (*TxString == '\0') {
			...
			while(!(Get_Bit(SPI_StatusReg, InterruptFlag_BIT)));
			break;
			} else {
